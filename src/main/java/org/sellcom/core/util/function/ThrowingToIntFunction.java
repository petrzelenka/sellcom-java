...
/**
 * Represents a function that accepts a single argument and produces an {@code int}-valued result, possibly throwing an exception.
 *
 * @since 1.0
 */
@FunctionalInterface
public interface ThrowingToIntFunction<T> extends ToIntFunction<T> {
	 ...
	 * @since 1.0
	 */
	int applyAsIntThrowing(T argument) throws Exception;

	/**
	 * Returns a {@code ToIntFunction} applying the given function to the argument if this function throws an exception.
	 *
	 ...
	 * @since 1.0
	 */
	default ToIntFunction<T> fallbackTo(ToIntFunction<T> fallback, Consumer<Exception> exceptionConsumer) {
	...
	/**
	 * Returns a {@code ToIntFunction} returning the given value if this function throws an exception.
	 *
	 * @since 1.0
	 */
	 ...
	 * @since 1.0
	 */
	default ToIntFunction<T> orReturn(int value, Consumer<Exception> exceptionConsumer) {
		return argument -> {
			try {
				return applyAsIntThrowing(argument);
			} catch (Exception e) {
	...
	/**
	 * Returns a {@code ThrowingToIntFunction} throwing an exception of the given type if this function throws an exception.
	 ...
	 * @since 1.0
	 */
	default ThrowingToIntFunction<T> orThrow(Class<? extends RuntimeException> exceptionClass) {
	 ...
	 * @since 1.0
	 */
	default ThrowingToIntFunction<T> orThrow(Class<? extends RuntimeException> exceptionClass, String message) {
		Contract.checkArgument(exceptionClass != null, "Exception class must not be null");
	...
	/**
	 * Returns a {@code ThrowingToIntFunction} applying the given function to the argument if this function throws an exception.
	 *
	 * @throws IllegalArgumentException if {@code other} is {@code null}
	 *
	 * @since 1.0
	 */
	default ThrowingToIntFunction<T> orTryWith(ThrowingToIntFunction<? super T> other) {
	 ...
	 * @since 1.0
	 */
	default ThrowingToIntFunction<T> orTryWith(ThrowingToIntFunction<? super T> other, Consumer<Exception> exceptionConsumer) {
