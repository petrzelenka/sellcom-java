...
public interface ThrowingSupplier<T> extends Supplier<T> {
	...
	/**
	 * Returns a {@code Supplier} returning from the given supplier if this supplier throws an exception.
	 *
	 ...
	 * @since 1.0
	 */
	default Supplier<T> fallbackTo(Supplier<T> fallback, Consumer<Exception> exceptionConsumer) {
	...
	/**
	 * Returns a {@code Supplier} returning the given value if this supplier throws an exception.
	 *
	 * @since 1.0
	 */
	default Supplier<T> orReturn(T value) {
	 ...
	 * @since 1.0
	 */
	default Supplier<T> orReturn(T value, Consumer<Exception> exceptionConsumer) {
		return () -> {
			try {
				return getThrowing();
			} catch (Exception e) {
	...
	/**
	 * Returns a {@code ThrowingSupplier} throwing an exception of the given type if this supplier throws an exception.
	 ...
	 * @since 1.0
	 */
	default ThrowingSupplier<T> orThrow(Class<? extends RuntimeException> exceptionClass) {
	 ...
	 * @since 1.0
	 */
	default ThrowingSupplier<T> orThrow(Class<? extends RuntimeException> exceptionClass, String message) {
		Contract.checkArgument(exceptionClass != null, "Exception class must not be null");
	...
	/**
	 * Returns a {@code ThrowingSupplier} returning from the given supplier if this supplier throws an exception.
	 *
	 * @throws IllegalArgumentException if {@code other} is {@code null}
	 *
	 * @since 1.0
	 */
	default ThrowingSupplier<T> orTryWith(ThrowingSupplier<? extends T> other) {
	 ...
	 * @since 1.0
	 */
	default ThrowingSupplier<T> orTryWith(ThrowingSupplier<? extends T> other, Consumer<Exception> exceptionConsumer) {
